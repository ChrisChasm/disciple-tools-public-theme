...
add_action( 'admin_enqueue_scripts', 'dtps_multi_role_admin_register_scripts', 0 );
add_action( 'admin_enqueue_scripts', 'dtps_multi_role_admin_register_styles', 0 );

# Custom manage users columns.
add_filter( 'manage_users_columns', 'dtps_multi_role_manage_users_columns' );
add_filter( 'manage_users_custom_column', 'dtps_multi_role_manage_users_custom_column', 10, 3 );
 ...
 * @return void
 */
function dtps_multi_role_admin_register_scripts() {
    ...
    wp_localize_script( 'members-edit-role', 'dtps_multi_role_i18n', $i18n );
}
 ...
 * @return void
 */
function dtps_multi_role_admin_register_styles() {
 ...
 * @return void
 */
function dtps_multi_role_delete_role( $role ) {
    ...
    dtps_multi_role_role_factory()->remove_role( $role );
}
 ...
 * @return array
 */
function dtps_multi_role_get_user_meta_keys() {
    global $wpdb;
 ...
 * @return array
 */
function dtps_multi_role_manage_users_columns( $columns ) {

    // If multiple roles per user is not enabled, bail.
    if ( ! dtps_multi_role_multiple_user_roles_enabled() ) {
        return $columns;
    }
 ...
 * @return string
 */
function dtps_multi_role_manage_users_custom_column( $output, $column, $user_id ) {

    if ( 'roles' === $column && dtps_multi_role_multiple_user_roles_enabled() ) {
        ...
        if ( is_array( $user->roles ) ) {
            ...
            foreach ( $user->roles as $role ) {
                ...
                if ( dtps_multi_role_role_exists( $role ) ) {
                    $user_roles[] = dtps_multi_role_translate_role( $role );
                }
            }
